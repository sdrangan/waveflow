...
Owns exactly the ten rows of the table above.  Per point it returns a record of the shape both scripts
already produce — `{ok, elapsed, error?, top?, module_sum?, integration?, **params}` — so the summary
format does not change and existing `results/sweep.json` files stay readable.
...
3. **Does resume still need a summary file?**  A point whose records are in the store is, by
   definition, done — so resume could consult the store rather than `sweep.json`, and the summary
   could become a pure log.  Against: a point may file several module records, a dry run files none,
   and **failures are not in the store at all**, so a store-only resume would retry every failure
   forever.  Probably: resume from the summary, and treat store-vs-summary disagreement as a warning.
...
5. **Should the summary keep `grid`?**  Both write the axes into the JSON.  Useful for reading a
   summary standalone; redundant with the points themselves.  Keep it — P0 demands byte-identical
   output anyway, and a summary that cannot say what space it covered is harder to interpret than one
   that repeats itself.
