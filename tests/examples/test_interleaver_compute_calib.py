...
def test_fit_load_predict_roundtrip(tmp_path):
    ...
    assert c.n == 256
    assert c.timing.predict({"n": 256}) == pytest.approx(400.0, abs=0.1)
    assert c.timing.predict({"n": 128}) == pytest.approx(200.0, abs=0.1)
...
def test_seed_used_without_calib(tmp_path):
    ...
    assert c.timing.predict({"n": 256}) == pytest.approx(expect, abs=0.1)
