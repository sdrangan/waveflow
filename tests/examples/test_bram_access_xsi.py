"""The RTL gate for ``examples/bram_access`` — ``plans/bram_access.md`` Stages 2 and 3.

What xsim elaborates is the **wrapper** (``bram_access_top``): the kernel plus its hand-written
...
text output (measured four ways — ``plans/bram_access.md`` § *DECIDED 2026-08-25*).  Five shipped
...
:func:`~examples.bram_access.bram_access.collision_scenario` must come back dirty.
"""
...
from examples.bram_access.bram_access_build import (
    RTL_FILES,
    TOP,
    WRAPPER,
    generate_tb,
...
#: function it instantiates, so there is no ``bram_access_`` prefix here even though the RTL file on
#: disk carries one.
...
def _run_traced(sc, keep: Path) -> Path:
    ...
    assert "XSI_EXITCODE=0" in out, f"bram_access XSI run did not complete cleanly:\n{out[-3000:]}"
...
def test_scenario_zero_has_no_hazard(runs):
    ...
    memory's own word and is what is available (``plans/bram_access.md`` § *DECIDED 2026-08-25*).
    """
...
def test_the_memory_answers_exactly_read_latency_cycles_later(runs):
    ...
    comp = elaborate(BramAccess, {"bitwidth": 64, "depth": DEPTH}, name=TOP)
...
def test_the_two_backends_agree_on_rate_and_the_model_pays_the_fill(runs):
    ...
    * **the first word does not match for free.**  It is off by exactly ``READ_LATENCY`` unless the
    ...
    """
    from examples.bram_access.bram_access import run_pysim
