...
```bash
source ../pysilicon-venv/bin/activate   # the repo venv; see CLAUDE.md
pytest tests/vitis_l1/gemv/ -q
```
...
```bash
source ../pysilicon-venv/bin/activate   # the repo venv; see CLAUDE.md
cd tests/vitis_l1/gemv
bash cpp/regen_golden.sh
...
cd tests/vitis_l1/gemv/verifyGEMV
...
`-ffp-contract=off` pin in `cpp/regen_golden.sh` **matches the hardware**, rather than being a
