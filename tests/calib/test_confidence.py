...
def test_overdetermined_exact_fit_reports_exact():
    ...
    conf = m.confidence_feat({"n": 64})          # inside the fitted span
...
def test_leaving_the_sampled_region_outranks_a_confirmed_form():
    ...
    """
    conf = _model().fit(_affine(6)).confidence_feat({"n": 100_000})
...
def test_extrapolating_a_noisy_fit_is_distinguishable_from_an_exact_one():
    df = _affine(6)
    df.loc[2, "cycles"] += 7.0
    conf = _model().fit(df).confidence_feat({"n": 100_000})
    assert conf.level is ConfidenceLevel.EXTRAPOLATED
...
def test_underdetermined_fit_is_refused_exactness():
    ...
    conf = m.confidence_feat({"n": 24})          # inside, so the level turns on exactness alone
...
def test_extrapolation_wins_over_exactness():
    ...
    m = _model().fit(_affine(6))
    assert m.confidence_feat({"n": 5}).level is ConfidenceLevel.EXTRAPOLATED


def test_unfitted_model_is_uncalibrated():
    assert _model().confidence_feat({"n": 32}).level is ConfidenceLevel.UNCALIBRATED
...
def test_estimate_pairs_the_value_with_confidence():
    ...
    assert est.value == pytest.approx(m.predict_feat({"n": 64}))
...
def test_fit_summary_survives_save_and_load(tmp_path):
    ...
    assert deployed.confidence_feat({"n": 500}).level is ConfidenceLevel.EXTRAPOLATED
    assert deployed.confidence_feat({"n": 64}).level is ConfidenceLevel.EXACT
...
def test_artifact_without_a_summary_still_loads(tmp_path):
    ...
    assert m.predict_feat({"n": 10}) == pytest.approx(30.0)
    assert m.confidence_feat({"n": 10}).level is ConfidenceLevel.UNCALIBRATED
...
def test_lookup_never_claims_exact():
    ...
    m = _interp()
    conf = m.confidence_feat({"n_col": 128})
    assert conf.level is ConfidenceLevel.INTERPOLATED
...
def test_lookup_declares_its_clamp_when_out_of_range():
    ...
    conf = _interp().confidence_feat({"n_col": 4096})
    assert conf.level is ConfidenceLevel.EXTRAPOLATED
...
def test_timing_model_exposes_confidence_and_tags_the_component(tmp_path):
    ...
    conf = tm.confidence_feat({"nwords": 128, "num_trans": 8})
    ...
    assert est.value == pytest.approx(tm.predict_feat({"nwords": 128, "num_trans": 8})[0])
    assert est.source == "pysim"
...
def test_asking_for_confidence_does_not_change_the_prediction(tmp_path):
    ...
    before = tm.predict_feat(row)
    tm.confidence_feat(row)
    assert tm.predict_feat(row) == before
...
def test_a_params_only_artifact_reports_uncalibrated_but_says_why(tmp_path):
    ...
    conf = m.confidence_feat({"n": 64})
    assert conf.level is ConfidenceLevel.UNCALIBRATED
    assert conf.facts["has_fitted_params"] is True
    assert conf.facts["from_seed"] is False
    assert m.predict_feat({"n": 64}) == pytest.approx(138.0)     # the number is still real
