...
The ring is backed by a real ``MemoryMod`` (decision 9): ``inline=False`` plus
...
def _make_mem(sim, layout, clk):
    """A ``MemoryMod`` backing *layout*'s ring region.
    ...
    mem = MemoryMod(sim=sim, word_size=layout.mem_bw, inline=False, clk=clk)
    mem.alloc(total_words)
    return mem


def _make_queue_direct(capacity, *, elem_words=1, mem_bw=32, base_addr=0x0):
    """One AXIMMQueue over a DirectMMIF + MemoryMod; returns (sim, queue)."""
    sim = Simulation()
    clk = Clock(freq=1.0)
...
def _make_spsc_crossbar(sim, layout, clk, *, latency_init=0.0, latency_read_return=0.0):
    """Producer/consumer AXIMMQueues over a 2-master crossbar + MemoryMod.
