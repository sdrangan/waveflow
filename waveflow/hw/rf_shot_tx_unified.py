r"""rf_shot_tx_unified.py — **one** shot transmitter, on the lock, with both play modes.

``plans/rf_shot_unify.md`` Stage A.  The merge of :mod:`waveflow.hw.rf_shot_tx` (finite play,
``ShotPhase`` + ``rdy`` + ``done``, five tasks) and :mod:`waveflow.hw.rf_shot_loop` (infinite play,
``LockedT2pMemIF``, three tasks) into a single design that does what both did::
...
**The two predecessors are complementary, not duplicates**, which is why this is a build and not a
rename:

===============================  =======================  ==========================
                                 ``RfShotTx``             ``RfShotTxLoop``
===============================  =======================  ==========================
...
and preempts otherwise.  That asymmetry is why the ``done`` token survives from ``RfShotTx``: the
...
----------------------
``RfShotTx`` hand-wires seven internal channels and two :class:`~waveflow.hw.bram.BramIF`\ s, and
carries a :class:`~waveflow.hw.rf_shot_buf.ShotPhase` object whose docstring says it is **pysim-only**
— so its central safety claim never had an RTL witness.  Here one ``add_if(lock)`` files the two lock
...
_OpField = IntField.specialize(bitwidth=8, signed=False)
_IdxField = IntField.specialize(bitwidth=IDX_BW, signed=False)
...
class ShotTxLoader(FreeRunMod):
    ...
    ``RfShotTxLoop``'s memory write in one body: there is no separate buffer task, because the lock
    ...
    **``busy`` covers both opcodes.**  A ``SHOT_LOOP`` arriving while a *finite* shot plays is
    ...
    def run_iter(self) -> ProcessGen[None]:
        ...
        **A known twin divergence, carried over from ``rf_shot_loop`` and re-stated because it is
        ...
        # played, on EITHER path.  `RfShotTxLoop` plays the padded result because it has no way to
...
class ShotTxPlayer(FreeRunMod):
    ...
    **The play loop differs from ``RfShotTxLoop``'s in exactly one place**, and it is the wrap::
...
@dataclass
class RfShotTxUnified(FreeRunMod):
    ...
    **Named ``RfShotTxUnified`` only until Stage B**, which deletes ``rf_shot_tx.py`` and
    ``rf_shot_loop.py`` and renames this module to ``rf_shot_tx.py`` and this class to ``RfShotTx``.
    Stage A must not touch either, so the two names coexist for exactly one stage.

    What the lock removed, counted
    ------------------------------
    ``RfShotTx`` wires **seven** internal channels and **two** ``BramIF``\ s by hand
    (``pay rep rdy_load rdy_play dense samp done`` + ``bufw bufr``), instantiates **five** tasks, and
    shares a :class:`~waveflow.hw.rf_shot_buf.ShotPhase` object between two of them.  This wires
    **three** (``rep done samp``) and one ``add_if(lock)``, and instantiates three.  The four that
    vanished — ``pay``, ``rdy_load``, ``rdy_play``, ``dense`` — existed only to move samples between
    tasks the lock made unnecessary, and ``ShotPhase`` is replaced by a guard that also exists at RTL.
    ...
    #: copying the composite.  See ``rf_shot_loop``'s ``player_cls`` for the argument.
    ...
    def for_word(cls, word, *, depth: int = BUF_DEPTH, nword: int = SHOT_WORDS, **kwargs):
        ...
        if not (isinstance(word, type) and issubclass(word, RfdcSampWord)):
            raise TypeError(
                f"RfShotTxUnified.for_word() takes the converter's WORD TYPE — the packing "
