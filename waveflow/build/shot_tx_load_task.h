...
//
// `s_in` and `resp_out` are `streamutils::axi4s_word` streams -- `ap_axis<W,0,0,0>` -- so the kernel
// really has `s_in_TLAST` and `resp_out_TLAST` pins.  Every other free-running top in this repo has
// plain `ap_uint` boundary streams and therefore no such pin; see
// waveflow.hw.interface.FramedStreamIFSlave for how a design asks.
//
// It has to be `ap_axis` and not the plain `{data, last}` `framed_word`, and that is MEASURED
// (2026-08-31) rather than assumed: a framed_word boundary port compiles, and Vitis packs the whole
// struct into one wide TDATA -- at W=64 the port came out `[127:0] s_in_TDATA` with no TLAST
// anywhere, and the wrapper failed to elaborate against a pin that was never emitted.  The side
// channels are a property of ap_axis, not of having a `last` member.  (framed_word stays right for
// an INTERNAL channel, where ap_axis is refused outright -- HLS 214-208.)
//
// The pin is here because a payload word and a header word are the same W bits: without a frame
// boundary there is no in-band way to say "that was the end", so a host that sends fewer words than
// its header declared would simply stall the buffer's counted loop.  A hang is indistinguishable
// from a deadlock, and it is invisible from the host side because the DMA reports success either
// way.  With the pin, a short transfer is a VERDICT ON THE DATA PATH -- which is the whole reason
// § "The response is not optional" exists.
//
...
static void shot_tx_load_task(hls::stream<streamutils::axi4s_word<W> >& s_in,
...
#pragma HLS reset variable=busy
    ...
    // so a comment edit above renames the synthesized module and the report entry with it -- and a
    // gate that looks the II up by name then MISSES and skips, which reads as a pass.  Both loops
    // here were unlabelled once and the rename happened on the first edit after they were recorded.
take_shot:
    for (int i = 0; i < NW; i++) {
