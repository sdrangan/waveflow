...
| `MemComponent` | the one arena **both** `m_axi` bundles reach — `m_in` reads `P` and `X`, `m_out` writes `Y` |
...
The memory is a flat `MemComponent`; the harness carves it into per-job `P` / `X` / `Y` buffers by
