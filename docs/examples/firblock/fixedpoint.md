...
sweep in [Parameter sweep](./sweep.md) runs `W ∈ {8, 16, 24}` and would hit the ceiling at `W = 32`
with 32 taps, which is why that point is absent rather than merely uninteresting.
...
- [Parameter sweep](./sweep.md) — what changing `W` costs in DSPs.
