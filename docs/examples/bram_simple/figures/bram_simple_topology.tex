% bram_simple_topology.tex — ONE source, ONE rendered SVG (bram_simple_topology.svg), embedded in
% ../index.md.
%
...
%
% overview.md carries the same topology as a Mermaid flowchart, which is the right tool there (cheap
% to edit, and that page is about the structure).  This is the index page's figure, where a rendered
% SVG reads better.
%
...
\node[lbl, above=0.8mm of kernel.north, text=nyuviolet, font=\sffamily\scriptsize\bfseries]
  {\texttt{bram\_simple} --- the Vitis kernel};
...
\draw[ln] (wr.east) -- ++(17mm,0) |- ([yshift=8mm]mem.west);
\node[port] at ([xshift=9mm, yshift=3.2mm]wr.east) {buf\_w};
\node[sub]  at ([xshift=9mm, yshift=-2.6mm]wr.east) {mode=bram};

\draw[ln] ([yshift=-8mm]mem.west) -- ++(-17mm,0) |- (rd.east);
\node[port] at ([xshift=9mm, yshift=3.2mm]rd.east) {buf\_r};
\node[sub]  at ([xshift=9mm, yshift=-2.6mm]rd.east) {mode=bram};
...
\node[lbl, below=0.8mm of wrap.south, text=nyumid, font=\sffamily\scriptsize\bfseries]
  {\texttt{bram\_simple\_top} --- the wrapper a simulator elaborates};
