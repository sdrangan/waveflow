...
Following the diagram in the [previous page](./index.md), the BRAM simple module exposes two 
...
Everything below is to be deleted or moved


===

[Move this to python.md where we actually define the modules and their interfaces]
...
[I think this section can be moved to the codegen.md page.  From a *python* perspective, there is a single top-level]
## BRAM are outside the Vitis top-level

In Waveflow, the BRAM is a separate hardware module.  So, in the simple example,  
We cannot directly instantiate BRAM in a Vitis top-level kernel and then connect that BRAM to multiple `hls::tasks`.  A techical explanation of the reason is described in [the interface guide](../../guide/interface/bram.md#why-a-shared-memory-cannot-live-inside-a-kernel) and related to streaming and synchornization protocols.    
...
[I think we can eliminate this figure, since we have a nicer figure on index.md ]
...
[Deleted text:  Other types of storage are already discussed in the index.md]
...
[This is related to the testbench details.  Explain later.]
