...
The same `BramAccess` class that ran in SimPy is the source three artifacts are generated from: the
...
python bram_access_build.py --through csynth
```
...
```python
from examples.bram_access.bram_access import SCHEMA_CLASSES

for cls in SCHEMA_CLASSES:
    print(f"{cls.__name__:19s} -> include/{cls.include_filename}")
```
...
- **`ram_1p` changes the port list, and the wrapper follows.** A `ram_1wnr` port emits all fourteen
  signals (an A/B pair of seven); `ram_1p` emits only the A half, so `bram_access_top.v`
...
spec = composite_top_spec(elaborate(BramAccess, {"bitwidth": 64, "depth": 1024},
                                    name="bram_access"), width=64)
for p in spec.ports:
    if p.kind == "bram":
...
```
ap_uint<64> buf_w[1024] | #pragma HLS INTERFACE mode=bram port=buf_w storage_type=ram_1p latency=1
...
rep = Path("examples/bram_access/bram_access_proj/solution1/syn/report/csynth.xml")
...
The last thing `csynth` does is re-emit `rtl_bram_access_top.f` from the RTL that is actually on
