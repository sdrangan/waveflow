...
The [model is already installed](./vitis_resmod.md), and two of its four counters already work: DSP
and BRAM follow from the declaration alone. LUT and FF do not, and until they have measurements the
model reports them as `UNCALIBRATED`. This is where they get them.

One synthesis gives you one point. A fit needs a grid, and which grid you choose decides what you are
able to learn — a sweep that only samples one regime will validate a law it never tested.
...
*(BRAM18 blocks. DSP is `LW` at every one of the 16 points — and both counters are exactly what the
[declaration already predicted](./vitis_resmod.md#before-any-sweep) before this sweep ran. LUT and FF,
the two that needed it, are on the [next page](./resmodfit.md).)*
...
shapes rather than a bit budget — see [`bram_estimate`](./vitis_resmod.md#memarray).
...
it is [predicted by the device rule](./resmodfit.md#the-corner-predicted-not-absorbed) rather
...
- [How well it fits](./resmodfit.md) — what these 16 points do for LUT and FF, checked against points
  held out of the fit.
