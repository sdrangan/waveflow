...
% Drawn at ONE channel, so the fabric ports are rx_streams[0] / tx_streams[0]; how they multiply
% is rf_channels.tex's claim, not this one.
...
\draw[ln] (adc.west) -- node[lbl, above] {\texttt{rx\_streams[0]}}
                        node[sub, below] {words} (dut.east |- adc);

% --- TX: your logic -> DAC -> environment (left to right) ----------------------------------------
\draw[ln] (dut.east |- dac) -- node[lbl, above] {\texttt{tx\_streams[0]}}
