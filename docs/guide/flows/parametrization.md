...
summary: "Parameterizing a HwComponent (both flows): HwParam[T] for per-instance, configurable synthesis knobs (bus widths, datapath sizing — vary per instance and per generated kernel variant) and HwConst[T] for class-level, fixed structural constants (static array extents). The crux is per-instance-configurable vs class-level-fixed. param_supports declares a set of HwParam values to emit as kernel variants; its C++ realization is comp_codegen/templating."
---

# Parameterization

Both flows build on a `HwComponent`, and either kind of component can be parameterized — the synthesis
surface lives on `HwComponent` itself, so this page applies to a `HostActivated` kernel and a
`FreeRunComp` alike.

## Why parameterize

A [`HwComponent`](./components.md) is a *template* for hardware, not a single fixed block. Parameterizing
...
[`HwParam[T]`](../../../waveflow/hw/hw_component.py) marks a dataclass field as a synthesis parameter
...
- **Identity-preserving for codegen.** `HwComponent.__post_init__` wraps each `HwParam` value as an
  [`HwParamValue`](../../../waveflow/hw/hw_component.py) — an `int` subclass that remembers the
...
- **Immutable after construction.** `HwComponent.__setattr__` raises if you reassign a `HwParam` field
...
[`HwConst[T]`](../../../waveflow/hw/hw_component.py) marks a **class attribute** that is fixed for the
...
immutability — the marker signals intent). [`discover_hw_const(cls)`](../../../waveflow/hw/hw_component.py)
...
[`param_supports`](../../../waveflow/hw/hw_component.py): a map of *variant key → `HwParam` overrides*.
...
`my_kernel_bw128`. [`validate_param_supports`](../../../waveflow/hw/hw_component.py) checks the keys
...
- [Hardware components](./components.md) — where these fields are declared on the class.
