#ifndef WAVEFLOW_GEN_RF_BLK_DELAY_TB_HARNESS_H
#define WAVEFLOW_GEN_RF_BLK_DELAY_TB_HARNESS_H
// rf_blk_delay_tb_harness.h -- GENERATED by waveflow (build/composite_gen.py::render_tb_harness)
...
#include "rf_blk_delay_ports.h"

namespace rf_blk_delay_tb {
...
struct Harness {
    ...
    explicit Harness(const std::string& wdb)
      : sim(rf_blk_delay_ports::DESIGN_DLL, wdb),
        xsi_tb_adc_if(2),
        xsi_tb_dac_if(2),
        s_in(sim.dut(), rf_blk_delay_ports::s_in, xsi_tb_adc_if, RfdcFormat{14, 4, 1.0, 16, 2, RFDC_REAL, RFDC_I_LOW}, 0.25),
        rx_resp(sim.dut(), rf_blk_delay_ports::rx_resp),
        tx_resp(sim.dut(), rf_blk_delay_ports::tx_resp),
        s_out(sim.dut(), rf_blk_delay_ports::s_out, xsi_tb_dac_if, RfdcFormat{14, 4, 1.0, 16, 2, RFDC_REAL, RFDC_I_LOW}, 0.25, 256),
        xsi_tb_adc_if_tx(xsi_tb_adc_if, 256, 0),
        xsi_tb_dac_if_rx(xsi_tb_dac_if, 0)
    {
        ...
        sim.pin_low(rf_blk_delay_ports::ZERO_PORTS, rf_blk_delay_ports::ZERO_PORTS_N);
        ...
        participants_.push_back(&rx_resp);
        participants_.push_back(&tx_resp);
        ...
        rx_resp.out_bundle = "vectors/rxresp";
        tx_resp.out_bundle = "vectors/txresp";
...
#endif  // WAVEFLOW_GEN_RF_BLK_DELAY_TB_HARNESS_H
