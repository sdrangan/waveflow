...
if {$start_at eq "csim"} {
    open_project -reset waveflow_conv2d_df_proj
    set_top conv2d_df
    add_files conv2d_df.cpp
    add_files -tb conv2d_tb.cpp -cflags "-Wno-unknown-pragmas -DWAVEFLOW_CONV2D_DF"
    ...
    open_project waveflow_conv2d_df_proj
    open_solution "solution1"
}

if {$start_idx <= 0 && $through_idx >= 0} {
    if {[catch {csim_design -argv "$data_dir"} res]} {
        puts "WAVEFLOW_ERROR: conv2d_df C-Simulation failed."
        puts $res
        exit 1
    }
}
...
if {$start_idx <= 1 && $through_idx >= 1} {
    if {[catch {csynth_design} res]} {
        puts "WAVEFLOW_ERROR: conv2d_df C-Synthesis failed."
        puts $res
        exit 1
    }
}
...
if {$start_idx <= 2 && $through_idx >= 2} {
    if {[catch {cosim_design -argv "$data_dir" -trace_level $trace_level} res]} {
        puts "WAVEFLOW_ERROR: conv2d_df RTL Co-Simulation failed."
        puts $res
        exit 1
    }
}

puts "WAVEFLOW_SUCCESS: conv2d_df stages $start_at through $through passed."
exit 0