...
- uses the **retired array-serialization API** (the old `read_array` / `write_array` bulk and
  ...
  style** (hand-written Vitis C++ driven by a `run.tcl`, not an `HwModule` → codegen → hook), so
...
`run.tcl`) — it predates the `HwModule` model and is the last example that called the old bulk
...
It will **return rewritten from scratch** as a proper `HwModule` once the systolic array exists,
