...
HAND_WRITTEN_TASKS = ("fir_cmd_rx_task.h", "fir_compute_task.h")
...
def render_types_h(ntap: int, samp_w: int, samp_i: int) -> str:
    """``fir_compute_types.h`` — the sample and accumulator C++ types, from the Python formats."""
    samp = samp_type(samp_w, samp_i)
    ...
    return f"""\
#ifndef WAVEFLOW_FIR_COMPUTE_TYPES_H
#define WAVEFLOW_FIR_COMPUTE_TYPES_H
...
namespace fir_compute_types {{
    typedef {samp.cpp_type} samp_t;   // W={samp_w}, I={samp_i}
...
#endif  // WAVEFLOW_FIR_COMPUTE_TYPES_H
"""
...
def gen_headers(config: BuildConfig, mem_dwidth: int = MEM_DW) -> None:
    """Schema headers + streamutils + memmgr + the fixed framed mem-stream bodies into ``include/``."""
    dag = BuildDag()
...
def generate_dut(out_dir: Path = HERE, mem_dwidth: int = MEM_DW, ntap: int = DEFAULT_NTAP,
                 samp_w: int = DEFAULT_SAMP_W, samp_i: int = DEFAULT_SAMP_I) -> Path:
    ...
    compute = elaborate(FirCompute, {"mem_dwidth": mem_dwidth, "ntap": ntap,
                                     "samp_w": samp_w, "samp_i": samp_i}, name="fir_compute")
    (inc / "fir_compute_types.h").write_text(render_types_h(ntap, samp_w, samp_i), encoding="utf-8")
    ...
    comp = elaborate(FirBlock, {"mem_dwidth": mem_dwidth, "ntap": ntap,
                                "samp_w": samp_w, "samp_i": samp_i}, name=TOP)
...
def check_xsi_outputs(xsi_dir: Path, program=None, blk: int | None = None, ntap: int = DEFAULT_NTAP,
    ...
    for j, step in enumerate(s for s in tb._steps if s["op"] == FirOp.FILTER):
        dst, n = step["dst"], step["n"]
        got, want = out[dst:dst + n], golden[dst:dst + n]
        if not np.array_equal(got, want):
            bad = int(np.argmax(got != want))
            raise AssertionError(
                f"fir_block RTL block {j} sample {bad}: 0x{int(got[bad]):08x} != "
