// Static-top FIR cosim/csim TB: load the Phase-1 fixtures into one mem image
// (X | h | Y), drive the command over the s_in AXI-stream, run the fir() top, drain the
// m_out response, and check the Y region bit-exact vs Y_golden.bin (the ONE shared
// fir_golden).  Control is AXI-stream (like shared_mem); the data stays on m_mem.
#include "fir.hpp"
#include <cstdint>
#include <cstdio>
#include <cstring>
...
int main(int argc, char** argv) {
    ...
    // Drive the command over the control stream (op, tx_id, x_off, h_off, y_off,
    // n_rows, n_cols — the FIRCmd field order), then run the kernel.
    hls::stream<ap_uint<32> > s_in;
    hls::stream<ap_uint<32> > m_out;
    s_in.write(ap_uint<32>(0));          // op = fir
    s_in.write(ap_uint<32>(0));          // tx_id
    s_in.write(ap_uint<32>(x_off));
    s_in.write(ap_uint<32>(h_off));
    s_in.write(ap_uint<32>(y_off));
    s_in.write(ap_uint<32>(n_rows));
    s_in.write(ap_uint<32>(n_cols));

    fir(s_in, m_out, mem.data());

    // Drain the response (tx_id echo + status word).
    (void)m_out.read();
    (void)m_out.read();
