// interleaver_sob_bfm_tb.cpp — XSI BFM for the P-SOB interleaver variant (gen/interleaver_sob.cpp):
// the symmetric topology where BOTH P and X are resident SOB blocks (Sequencer -> MemRStream ->
// SplitFill ->(p_blk,x_blk SOB)-> GatherTwoSob -> MemWStream).  Same boundary as the stream/SOB-mix
// interleaver, so this is byte-for-byte the interleaver BFM with only the elaborated-design path
// retargeted.  The decisive A/B test: does the symmetric two-SOB topology clear the nj=8 deadlock the
// stream-mix variant hits?  Bit-exact Y[i]=X[P[i]] + steady-state period (slope) = throughput.
//
...
// VERDICT (Phase 4b, the A/B de-risk): the P-SOB variant ALSO HANGS at nj=8 — done=6/8, even earlier
// than the stream/SOB-mix's 7/8 (steady-state 296 cyc/job, identical, jobs 0..5). So making P a
// resident SOB does NOT clear the deadlock: it is in the m_axi COMPOSITION (MemRStream read +
// MemWStream write + free-running compute tiles), not the P stream-vs-SOB choice. Striking pattern:
// done == (#hls::tasks)+1 -> mix has 6 tasks/7 done, P-SOB has 5 tasks/6 done. (sob3, 4 tasks &
// monolithic Cmd-chained load, passes nj=8, so the correlation is specific to this componentization.)
// nj<=6 PASS bit-exact at 296 cyc/job (verified nj=6 -> 6/6). See the phase summary.
...
int main() {
    ...
    std::printf("interleaver_sob XSI BFM: n=%d nj=%d cycles=%ld done=%d/%d\n", N, NJ, latency, done_count, NJ);
