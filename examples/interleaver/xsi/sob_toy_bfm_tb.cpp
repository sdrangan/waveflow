// sob_toy_bfm_tb.cpp — pure-AXIS XSI BFM that RTL-verifies the GENERATED free-running
// Fill ->SOBIF-> Gather composite (gen/sob_toy.cpp) in xsim.  The Phase-3 (Gate 3) harness: it proves
// the generated hls::task + stream_of_blocks top reproduces the hand-written sandbox
// interleaver_sob_task.cpp — bit-exact AND overlapping (~ (NJ+1)*N floor, not NJ*2N serial).
// Adapted verbatim from sandbox/sob/xsi_task/sob_bfm_tb.cpp; only the elaborated-design path and the
// data type differ (the kernel moves ap_uint<32> words — the float bit patterns are just 32-bit data,
// so the gather is bit-exact whatever the interpretation).  NO m_axi — three AXIS channels + clock:
//   x_in  (kernel slave, TB master):  NJ*N words,  X[j][k] = k*0.5 - 3 + j (as float bits)
//   p_in  (kernel slave, TB master):  NJ*N ints,   P[i]    = (i*13+5) % N
//   y_out (kernel master, TB slave):  NJ*N words,  expect  Y[j][i] = X[j][P[i]]
// Cycle protocol (per il_bfm): sample kernel outputs in the clock-LOW phase, detect beats
// (VALID && READY), pulse the edge, advance FSMs.  Reports cycle count vs the overlap floor.
#include <cstdio>
#include <cstring>
#include <cstdint>
#include <string>
...
int main() {
    ...
    std::printf("sob_toy XSI BFM: n=%d nj=%d cycles=%ld  (serial~%d, overlap_floor~%d)\n",
