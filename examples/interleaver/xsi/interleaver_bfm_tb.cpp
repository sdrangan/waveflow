// interleaver_bfm_tb.cpp — XSI cycle-based BFM that RTL-verifies the GENERATED word-granular
// interleaver (gen/interleaver.cpp) in xsim.  The Phase-4 (Gate 4) harness: it proves the generated
// 6-hls::task composite (Sequencer -> MemRStream -> Demux -> Fill ->SOBIF-> GatherWord -> MemWStream)
// reproduces the hand-written sandbox interleaver_task_sob3.cpp — bit-exact Y[i]=X[P[i]] AND the same
// steady-state throughput (~n/job ~= 295 for n=256, MEM_DW=64: the load-bus floor of 2*(n/LW) reads).
//
...
// STATUS (Gate 4): nj<=7 PASS bit-exact at 296 cyc/job (== sob3's ~295, the load-bus floor). nj=8
// hangs at 7/8 with all buses idle — a Vitis hls::task+m_axi endurance limit in this 6-task
// composite that is INSENSITIVE to every dataflow knob tried (p_words / SOBIF / command-FIFO depths,
// num_read/write_outstanding — all give byte-identical done cycles). The monolithic sob3 (4 tasks,
// Cmd chained through the pipeline) passes nj=8 here; the componentized graph does not. Correctness
// and throughput are proven; the nj=8 endurance is the open item (see the phase summary).
...
int main() {
    ...
    char wdb[] = "interleaver_bfm.wdb"; info.wdbFileName = wdb;
    xsi.open(&info);
    Dut d(xsi);
