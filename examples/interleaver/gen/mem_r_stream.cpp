// mem_r_stream.cpp — GENERATED (template codegen, examples/interleaver/mem_stream_gen.py).
// DO NOT EDIT: the fixed body is the validated sandbox AXI->stream (a2s) (interleaver_task_sob3.cpp).
//
// Free-running (ap_ctrl_none) single-hls::task memory endpoint, word-granular (ap_uint<MEM_DW>).
// The sole m_axi read owner touches ONLY streams (the DTLP /
...
// AXI->stream (a2s): dequeue one MRCmd, byte_addr -> word index, burst n_words words. Word rate.
static void mem_r_stream_task(hls::stream<word_t>& s_cmd,
                              const ap_uint<MEM_DW>* m_mem,
                              hls::stream<word_t>& m_out) {
    MRCmd c;
...
void mem_r_stream(hls::stream<word_t>& s_cmd,
                  const ap_uint<MEM_DW>* m_mem,
                  hls::stream<word_t>& m_out) {
...
#pragma HLS stable variable=m_mem
    hls_thread_local hls::task t(mem_r_stream_task, s_cmd, m_mem, m_out);
}
