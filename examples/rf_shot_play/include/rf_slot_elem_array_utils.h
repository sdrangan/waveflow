#ifndef INCLUDE_RF_SLOT_ELEM_ARRAY_UTILS_H
#define INCLUDE_RF_SLOT_ELEM_ARRAY_UTILS_H
...
namespace rf_slot_elem_array_utils {

using value_type = ap_int<16>;
static constexpr int value_bitwidth = 16;
...
struct read_array_elem_impl<64> {
    static value_type run_lane(const ap_uint<64>& w, int k) {
        #pragma HLS INLINE
        switch (k) {
            case 0: return (ap_int<16>)(w.range(15, 0));
            case 1: return (ap_int<16>)(w.range(31, 16));
            case 2: return (ap_int<16>)(w.range(47, 32));
            case 3: return (ap_int<16>)(w.range(63, 48));
        }
        return value_type();
    }
...
struct write_array_elem_impl<64> {
    static void write_lane(ap_uint<64>& w, int k, const value_type& v) {
        #pragma HLS INLINE
        switch (k) {
            case 0: w.range(15, 0) = v; break;
            case 1: w.range(31, 16) = v; break;
            case 2: w.range(47, 32) = v; break;
            case 3: w.range(63, 48) = v; break;
        }
    }
...
#endif // INCLUDE_RF_SLOT_ELEM_ARRAY_UTILS_H
