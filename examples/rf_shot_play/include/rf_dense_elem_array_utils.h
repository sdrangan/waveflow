#ifndef INCLUDE_RF_DENSE_ELEM_ARRAY_UTILS_H
#define INCLUDE_RF_DENSE_ELEM_ARRAY_UTILS_H
...
namespace rf_dense_elem_array_utils {

using value_type = ap_int<14>;
static constexpr int value_bitwidth = 14;
...
struct read_array_elem_impl<64> {
    static value_type run_lane(const ap_uint<64>& w, int k) {
        #pragma HLS INLINE
        switch (k) {
            case 0: return (ap_int<14>)(w.range(13, 0));
            case 1: return (ap_int<14>)(w.range(27, 14));
            case 2: return (ap_int<14>)(w.range(41, 28));
            case 3: return (ap_int<14>)(w.range(55, 42));
        }
        return value_type();
    }
...
struct write_array_elem_impl<64> {
    static void write_lane(ap_uint<64>& w, int k, const value_type& v) {
        #pragma HLS INLINE
        switch (k) {
            case 0: w.range(13, 0) = v; break;
            case 1: w.range(27, 14) = v; break;
            case 2: w.range(41, 28) = v; break;
            case 3: w.range(55, 42) = v; break;
        }
    }
...
struct read_stream_elem_impl<64> {
    static void run(hls::stream<ap_uint<64>>& s, value_type* out, int n) {
        ...
        ap_uint<64> w = s.read();
        if (n > 0) {
            out[0] = (ap_int<14>)(w.range(13, 0));
        }
        if (n > 1) {
            out[1] = (ap_int<14>)(w.range(27, 14));
        }
        if (n > 2) {
            out[2] = (ap_int<14>)(w.range(41, 28));
        }
        if (n > 3) {
            out[3] = (ap_int<14>)(w.range(55, 42));
        }
    }
};
...
#endif // INCLUDE_RF_DENSE_ELEM_ARRAY_UTILS_H
