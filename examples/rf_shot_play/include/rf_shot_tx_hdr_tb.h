#ifndef INCLUDE_RF_SHOT_TX_HDR_TB_H
#define INCLUDE_RF_SHOT_TX_HDR_TB_H
...
#define WAVEFLOW_ENABLE_RF_SHOT_TX_HDR_TB_H_MEMBERS
#include "rf_shot_tx_hdr.h"
#undef WAVEFLOW_ENABLE_RF_SHOT_TX_HDR_TB_H_MEMBERS

inline void ShotTxHdr::dump_json(std::ostream& os, int indent, int level) const {
...
inline void ShotTxHdr::load_json(const std::string& json_text, size_t& pos) {
    ...
    if (key == "opcode") {
        ...
        seen_root_nrepeat = true;
        this->nrepeat = static_cast<ap_uint<16>>(static_cast<unsigned long long>(streamutils::json_parse_number(json_text, pos)));
    }
    else {
    ...
    if (!seen_root_nrepeat) {
    throw std::runtime_error("Malformed JSON: missing required key 'nrepeat'.");
    }
}

inline void ShotTxHdr::load_json(std::istream& is) {
...
inline void ShotTxHdr::dump_json_file(const char* file_path, int indent) const {
...
inline void ShotTxHdr::load_json_file(const char* file_path) {
...
#endif // INCLUDE_RF_SHOT_TX_HDR_TB_H