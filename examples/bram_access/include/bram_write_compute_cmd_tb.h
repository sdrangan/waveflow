#ifndef INCLUDE_BRAM_WRITE_COMPUTE_CMD_TB_H
#define INCLUDE_BRAM_WRITE_COMPUTE_CMD_TB_H
...
#include "bram_op_tb.h"

#define WAVEFLOW_ENABLE_BRAM_WRITE_COMPUTE_CMD_TB_H_MEMBERS
#include "bram_write_compute_cmd.h"
#undef WAVEFLOW_ENABLE_BRAM_WRITE_COMPUTE_CMD_TB_H_MEMBERS

inline void WriteComputeCmd::dump_json(std::ostream& os, int indent, int level) const {
...
inline void WriteComputeCmd::load_json(const std::string& json_text, size_t& pos) {
    ...
    if (key == "tid") {
        ...
        this->opcode = static_cast<BramOp>(static_cast<long long>(streamutils::json_parse_number(json_text, pos)));
    }
    else if (key == "nsamp") {
        seen_root_nsamp = true;
        this->nsamp = static_cast<ap_uint<64>>(static_cast<unsigned long long>(streamutils::json_parse_number(json_text, pos)));
    }
    else if (key == "waddr") {
        seen_root_waddr = true;
        this->waddr = static_cast<ap_uint<64>>(static_cast<unsigned long long>(streamutils::json_parse_number(json_text, pos)));
    }
    else {
    ...
    if (!seen_root_waddr) {
    throw std::runtime_error("Malformed JSON: missing required key 'waddr'.");
    }
}

inline void WriteComputeCmd::load_json(std::istream& is) {
...
inline void WriteComputeCmd::dump_json_file(const char* file_path, int indent) const {
...
inline void WriteComputeCmd::load_json_file(const char* file_path) {
...
#endif // INCLUDE_BRAM_WRITE_COMPUTE_CMD_TB_H