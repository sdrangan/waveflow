#ifndef INCLUDE_BRAM_READ_CMD_TB_H
#define INCLUDE_BRAM_READ_CMD_TB_H
...
#define WAVEFLOW_ENABLE_BRAM_READ_CMD_TB_H_MEMBERS
#include "bram_read_cmd.h"
#undef WAVEFLOW_ENABLE_BRAM_READ_CMD_TB_H_MEMBERS

inline void ReadCmd::dump_json(std::ostream& os, int indent, int level) const {
    ...
    for (int i = 0; i < (level + 1) * step; ++i) { os << ' '; }
    os << "\"raddr\": ";
    os << static_cast<unsigned long long>(this->raddr);
    os << "\n";
    for (int i = 0; i < (level) * step; ++i) { os << ' '; }
    os << "}";
}

inline void ReadCmd::load_json(const std::string& json_text, size_t& pos) {
    streamutils::json_expect_char(json_text, pos, '{');
    bool seen_root_tid = false;
    bool seen_root_nsamp = false;
    bool seen_root_raddr = false;
    bool first = true;
    while (true) {
    ...
    if (key == "tid") {
        ...
        seen_root_raddr = true;
        this->raddr = static_cast<ap_uint<64>>(static_cast<unsigned long long>(streamutils::json_parse_number(json_text, pos)));
    }
    else {
    ...
    if (!seen_root_raddr) {
    throw std::runtime_error("Malformed JSON: missing required key 'raddr'.");
    }
}

inline void ReadCmd::load_json(std::istream& is) {
...
inline void ReadCmd::dump_json_file(const char* file_path, int indent) const {
...
inline void ReadCmd::load_json_file(const char* file_path) {
...
#endif // INCLUDE_BRAM_READ_CMD_TB_H