"""Committed-figure workflow for ``bram_access``'s timing pages.

Two figures, in the order ``docs/examples/bram_access/timing.md`` tells the story:
...
demand, via an explicit manifest — into ``docs/examples/bram_access/images/`` as committed assets, so
...
Run it with::
    ...
    python bram_access_build.py --through sync_docs_figures
...
matplotlib.rcParams["svg.hashsalt"] = "bram_access_figures"
...
FIGURE_MANIFEST = [
    {"name": "activity_full", "source": "results/activity_full.svg",
     "dest": "docs/examples/bram_access/images/activity_full.svg"},
    {"name": "activity_overlap", "source": "results/activity_overlap.svg",
     "dest": "docs/examples/bram_access/images/activity_overlap.svg"},
]
...
def render_activity_full(lane_list, path: Path) -> None:
    ...
    fig, ax, _ = ad.plot(
        ...
        title="bram_access — one scenario-zero run (the write port fills first; nothing reads "
...
class ActivityFiguresStep(BuildStep):
    ...
    description: str = "Render the bram_access activity figures from the traced RTL run."
...
class SyncDocsFiguresStep(BuildStep):
    ...
    @property
    def produces(self) -> dict:  # type: ignore[override]
        return {"docs_figures_sync": Path("docs/examples/bram_access/images/sync_status.json")}
    ...
    def run(self, config: BuildConfig, **_) -> dict[str, Any]:
        ...
        sync_path = repo_root / "docs" / "examples" / "bram_access" / "images" / "sync_status.json"
